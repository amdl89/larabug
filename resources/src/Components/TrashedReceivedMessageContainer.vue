<template>
  <v-container class="my-5 mx-auto">
    <v-row class="my-2">
      ...
      <v-col cols="3" sm="1" class="mx-auto">
        <bulk-delete-dialog
          title="Delete All Trashed Received Messages"
    ...
    <div class="mb-5 msg-list">
      <trashed-received-message-details-dialog
        :message="message"
        v-for="message in messages"
        :key="message.id"
      >
        <trashed-received-message
          :message="message"
          class="msg"
          @deleted="deleteMsg"
          @restored="deleteMsg"
...
import TrashedReceivedMessageDetailsDialog from "@/Components/TrashedReceivedMessageDetailsDialog.vue";
...
export default {
  ...
  components: {
    TrashedReceivedMessageDetailsDialog,
  ...
  methods: {
    ...
    async loadMessages() {
      ...
      try {
        ...
        const res = await axios.get(
          route("users.trashedReceivedMessages.index", {
            user: this.authUser,
    ...
    async restoreAll() {
      try {
        ...
        await axios.post(
          route("users.notInTrashReceivedMessages.storeAll", {
            user: this.authUser,
          })
        );
        ...
        this.$toasted.show("Restored All Trashed Received Messages", {
          type: "success",
          duration: 3000,
          icon: "check-bold",
        });
        ...
        this.$toasted.show("Failed To Restore Received Messages", {
          type: "error",
          duration: 3000,
          icon: "close",
        });
    ...
    async deleteAll() {
      try {
        ...
        await axios.delete(
          route("users.trashedReceivedMessages.destroyAll", {
            user: this.authUser,
          })
        );
        ...
        this.$toasted.show("Deleted All Trashed Received Messages", {
          type: "success",
          duration: 3000,
          icon: "check-bold",
        });
        ...
        this.$toasted.show("Failed To Delete Received Messages", {
          type: "error",
          duration: 3000,
          icon: "close",
        });
